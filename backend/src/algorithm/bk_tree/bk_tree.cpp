...
void BKTree::insertar(Nodo *nodo, const std::string &palabra,
                      long long frecuencia) {
...
void BKTree::busqueda(Nodo *nodo, const std::string &palabraBuscada, int radio,
                      std::vector<ResultadoBusqueda> &resultados) const {
  if (nodo == nullptr)
    return;
  ...
  if (distancia <= radio) {
    resultados.push_back(
        {nodo->palabra, nodo->frecuencia, distancia,
         distanciaLevenshteinNormalizada(palabraBuscada, nodo->palabra)});
  }
  ...
  for (const auto &hijo : nodo->hijos) {
    ...
    if (distanciaArista >= limiteInferior &&
        distanciaArista <= limiteSuperior) {
...
std::vector<ResultadoBusqueda>
BKTree::busqueda(const std::string &palabraBuscada, int radio) const {
